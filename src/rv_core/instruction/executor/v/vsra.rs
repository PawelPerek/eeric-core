...
pub fn vv(Opivv { vd, vs1, vs2, vm }: Opivv, v: &mut VectorRegisters, vec_engine: &VectorEngine) {
    ...
    .masked_map(
        ...
        |(vs2, vs1)| (vs2 as i64 >> shamt(vs1, vec_engine.sew)) as u64,
    )
...
pub fn vx(
    ...
    let vreg = v
        ...
        .masked_map(
            v.default_mask(vm, vec_engine),
            v.get(vd, vec_engine).iter_eew(),
            |vs2| (vs2 as i64 >> shamt(x[rs1], vec_engine.sew)) as u64,
        )
        .collect_with_eew(vec_engine.sew.clone());
...
pub fn vi(Opivi { vd, imm5, vs2, vm }: Opivi, v: &mut VectorRegisters, vec_engine: &VectorEngine) {
    let vreg = v
        ...
        .masked_map(
            v.default_mask(vm, vec_engine),
            v.get(vd, vec_engine).iter_eew(),
            |vs2| (vs2 as i64 >> shamt(imm5 as u64, vec_engine.sew)) as u64,
        )
        .collect_with_eew(vec_engine.sew.clone());
