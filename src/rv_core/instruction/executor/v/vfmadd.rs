...
pub fn vv(
    ...
    .masked_map(v.default_mask(vm, vec_engine), v.get(vd, vec_engine).iter_fp(), |(vs2, vs1, vd)| {
        (vd * vs1) + vs2
    })
    .collect_fp();
...
pub fn vf(Opfvf { vd, rs1, vs2, vm }: Opfvf, v: &mut VectorRegisters, vec_engine: &VectorEngine, f: &FloatRegisters) {
    let vreg = izip!(v.get(vs2, vec_engine).iter_fp(), v.get(vd, vec_engine).iter_fp())
        .masked_map(v.default_mask(vm, vec_engine), v.get(vd, vec_engine).iter_fp(), |(vs2, vd)| {
