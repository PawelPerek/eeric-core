...
pub fn wv(
    ...
    let vreg = izip!(v.get_wide(vs2, vec_engine).iter_fp(), v.get(vs1, vec_engine).iter_fp())
        .masked_map(
            v.default_mask(vm, vec_engine),
            v.get_wide(vd, vec_engine).iter_fp(),
...
pub fn wf(Opfvf { vd, rs1, vs2, vm }: Opfvf, v: &mut VectorRegisters, vec_engine: &VectorEngine, f: &FloatRegisters) {
    let vreg = v
        .get(vs2, vec_engine)
        .iter_fp()
        .masked_map(v.default_mask(vm, vec_engine), v.get_wide(vd, vec_engine).iter_fp(), |vs2| {
