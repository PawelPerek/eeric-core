...
mod tests {
    ...
    fn vseeg_basic() {
        ...
        v.apply(vs3    , vec![1, 2, 3, 4, 5, 6, 7, 8].into_iter().collect::<Vreg>(), &vec_engine);
        v.apply(vs3 + 1, vec![2, 4, 6, 8, 10, 12, 14, 16].into_iter().collect::<Vreg>(), &vec_engine);
        v.apply(vs3 + 2, vec![3, 6, 9, 12, 15, 18, 21, 24].into_iter().collect::<Vreg>(), &vec_engine);

        super::v(Vs { vs3, rs1, vm: false }, SEW::E8, 3, &v, &vec_engine, &x, &mut mem);
